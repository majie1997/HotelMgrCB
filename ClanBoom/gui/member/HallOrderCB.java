...
public class HallOrderCB extends JFrame implements ActionListener {

	private static final int width=450;
	private static final int height=300;
	private JLabel tipLabel=new JLabel("要预定的大厅餐桌号：");
	...
	public HallOrderCB(FirstMenuCB fs,ManUtil util,String userName){
	...
	//	util.initCompart();
		...
		if(util.DeskList.get(i).getDeskState()==0){
				...
				for(int i=0;i<util.DeskList.size();i++){
					if(util.DeskList.get(i).getDeskRecord().equals(number)){
						...
						if(util.DeskList.get(i).getDeskUse()==1){
								tipLabel.setText("该餐桌已被占用！");
							}else{
								tipLabel.setText("您成功预订该餐桌！");
								// add code here 1
